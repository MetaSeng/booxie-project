...
export default function SellScreen() {
  ...
  const [lastDetectionMessage, setLastDetectionMessage] = useState(
    "AI is ready to scan when the book is centered.",
  );
  ...
  const activeIndex = useMemo(
    () => SCAN_TYPES.findIndex((t) => t.id === activeTab),
    [activeTab],
  );
  const activeGuidance =
    SCAN_GUIDANCE[activeTab] || SCAN_GUIDANCE["Front Cover"];
  ...
  useEffect(() => {
    if (frontCoverData) {
      setFrontCoverData((prev: any) => ({
        ...
        type: listingType,
        price:
          listingType === "donation" ? 0 : prev.price === 0 ? 5.0 : prev.price,
      }));
    }
  }, [listingType]);
  ...
  useEffect(() => {
    ...
    if (container && activeIndex !== -1) {
      ...
      const scrollPos =
        activeIndex * cardWidth - container.offsetWidth / 2 + cardWidth / 2;
      ...
      navigate("/sell/edit", {
        state: {
          images: [
            ...
            backImageSrc ?? backCoverRef.current,
            receiptRef.current,
          ].filter(Boolean),
  ...
  const captureAndAnalyze = useCallback(
    async (isManual = false) => {
      ...
      if (!cameraReady) {
        setError(
          "Camera is not ready yet. Please allow camera access and try again.",
        );
        return;
      }
      ...
      try {
        ...
        if (!ai) {
          ...
          setError(
            "AI scanning is not configured. Add `VITE_GEMINI_API_KEY` to enable automatic detection.",
          );
          if (isManual) {
            if (activeTab === "Front Cover") {
              ...
              setLastDetectionMessage(
                "Front cover captured. Now scan the back cover.",
              );
              ...
              setLastDetectionMessage(
                "Back cover captured. Review and adjust your photos.",
              );
        ...
        if (activeTab === "Front Cover") {
          ...
          if (data?.detected) {
            setError("");
            setLastDetectionMessage(
              "Front cover detected. Preparing the next scan.",
            );
            setFrontCoverData({
            ...
            frontCoverRef.current = imageSrc;
            setLastDetectionMessage(
              "Front cover captured manually. Next: back cover.",
            );
            setActiveTab("Back Cover");
          } else if (!data) {
            setError(
              "AI scan returned an unreadable result. Tap the shutter to continue manually.",
            );
            setLastDetectionMessage(
              "The AI could not read this frame. Try moving closer or tapping the shutter.",
            );
          } else {
            setLastDetectionMessage(
              "No clear front cover yet. Keep the whole cover inside the guide.",
            );
          }
        } else if (activeTab === "Back Cover") {
          ...
          if (data?.detected) {
            ...
            backCoverRef.current = imageSrc;
            setLastDetectionMessage(
              "Back cover captured manually. Opening review.",
            );
            goToEditScreen(imageSrc);
          } else if (!data) {
            setError(
            ...
            setLastDetectionMessage(
              "The AI could not read this frame. Try pulling the book slightly closer.",
            );
          } else {
            setLastDetectionMessage(
              "No clear back cover yet. Keep the full back page inside the guide.",
            );
          }
        } else if (activeTab === "Receipt") {
          setReceiptImage(imageSrc);
        ...
        if (isGeminiQuotaError(err)) {
          setAutoScanEnabled(false);
          setError(
            "Gemini API quota exceeded. Please try again in 1 minute or use a different network.",
          );
          setLastDetectionMessage(
            "Auto scan paused because the AI quota was hit.",
          );
        } else if (isManual) {
          setError("Failed to scan. Please try again with better lighting.");
          setLastDetectionMessage(
            "Manual capture failed. Try brighter light and less glare.",
          );
        } else {
          setError(
            "Automatic AI scan failed. Tap the shutter to continue manually.",
          );
          setLastDetectionMessage(
            "Automatic scan failed. You can still capture manually.",
          );
        }
      } finally {
      ...
      <div className="flex-1 relative flex flex-col items-center justify-center min-h-0 px-6">
        <div className="w-full max-w-md mb-4 rounded-[28px] border border-white/15 bg-black/20 backdrop-blur-md px-5 py-4 shadow-xl">
          <div className="flex items-start justify-between gap-4">
            ...
            <div
              className={`shrink-0 rounded-full px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.16em] border ${isScanning ? "bg-[#32B38B] text-white border-[#6FE0BE]" : "bg-white/8 text-white/70 border-white/15"}`}
            >
        ...
        >
          <Webcam
            ...
            onUserMediaError={() => {
              ...
              setCameraError(
                "Camera access failed. Please allow camera permission or try another browser/device.",
              );
            }}
          ...
          {cameraError && (
            <div className="absolute inset-0 bg-black/70 flex flex-col items-center justify-center text-center px-6">
              ...
              <p className="text-xs text-white/80 mt-2">
                You can still continue after granting camera access and
            ...
            return (
              ...
              >
                ...
                <div className="relative z-10 flex flex-col items-center gap-1.5 w-full h-full p-2">
                  <div
                    className={`w-full h-[68%] rounded-[18px] flex items-center justify-center overflow-hidden transition-colors ${
                  ...
                  >
                    {scannedImage ? (
                      ...
                      <Icon
                        className={`w-[18px] h-[18px] transition-colors ${isActive ? "text-[#1A8765]" : "text-white/60"}`}
                      />
                    )}
                  </div>
                  <span
                    className={`text-[7px] font-black text-center leading-tight uppercase tracking-[0.14em] transition-colors ${
                ...
                {type.priority === "priority" && !scannedImage && (
                  <div
                    className={`absolute top-2 right-2 w-2 h-2 rounded-full ${isActive ? "bg-amber-400" : "bg-amber-400/50"} shadow-lg`}
                  />
                )}
                {scannedImage && (
